...
get_mvp_by_year <- function(year) {
  ...
  rookie_voting <- tables[[2]]
  
  "#AL_ROY_voting td , #AL_ROY_voting .right"
  
  ROY <- html_nodes(mlb_html, xpath = "//*[@id = AL_ROY_voting]")
...
colnames(CYA_DAT) <- as.character(CYA_DAT[1,])
MVP_DAT<- MVP_DAT|>
  filter(Year!= "Year")|>
  filter(Year >= 1911)|>
